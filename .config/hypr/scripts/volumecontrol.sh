...
notify_vol ()
{
    ...
    notify-send  -a "t2" -r 91190 -h string:x-canonical-private-synchronous:volume -t 800 -i "${ico}" "Volume:  ${vol}%" "${nsink}" 
}
...
notify_mute ()
{
    ...
    if [ "${mute}" == "true" ] ; then
        notify-send -a "t2" -r 91190 -h string:x-canonical-private-synchronous:volume -t 800 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        notify-send -a "t2" -r 91190 -h string:x-canonical-private-synchronous:volume -t 800 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
